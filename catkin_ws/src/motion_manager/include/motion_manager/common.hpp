...
//********Human hand*********//
const double THETA8_HOME_HumanHand = 0.0; /**< constant of the joint 8 (spread of the hand) in [rad] */
const double THETA8_FINAL_HumanHand = -double(M_PI)/2; /**< constant of the joint 8 (spread of the hand) in [rad] */
const double TOL_GRIP_HumanHand = 5.0; /**< tolerance on the grip in [mm] */


const int HAND_FINGERS_HumanHand = 3; /**< number of fingers per hand */
const int JOINTS_HAND_HumanHand = 4; /**< number of joints per hand */
const int N_PHALANGE_HumanHand = 3; /**< number of phalanges per finger */

//********Barrett hand*********//
const double THETA8_HOME_BarretHand = 0.0; /**< constant of the joint 8 (spread of the hand) in [rad] */
const double THETA8_FINAL_BarretHand = 0.0; /**< constant of the joint 8 (spread of the hand) in [rad] */
const double TOL_GRIP_BarretHand = 0.0; /**< tolerance on the grip in [mm] */
const int firstPartTorqueOvershootCountRequired_BarretHand = 1;/**< number of time that the torque applied to the first phalanx is bigger than firstPartMaxTorque */
const double firstPartMaxTorque_BarretHand = 0.9f;/**< max torque that can be applied to the first phalanx of the fingers */
const double closingOpeningTorque_BarretHand = 1.0f;/**< torque applied to the fingers when they are opening/closing */
const double closingVel_BarretHand = 60.0f * static_cast<double>(M_PI) / 180.0f; /**< joint velocity of the fingers when they are closing */
const double openingVel_BarretHand = -120.0f * static_cast<double>(M_PI) / 180.0f;/**< joint velocity of the fingers when they are opening */

const int HAND_FINGERS_BarretHand = 3; /**< number of fingers per hand */
const int JOINTS_HAND_BarretHand = 4; /**< number of joints per hand */
const int N_PHALANGE_BarretHand = 3; /**< number of phalanges per finger */

//********Electric gripper*********//
const double TOL_GRIP_Electric_Gripper = 0; /**< tolerance on the grip in [mm] */
const double TOL_TOOL_GRIP = - 0.315; /**< tolerance on the grip tool in [mm] */

const int HAND_FINGERS_Electric_Gripper = 2; /**< number of fingers per hand*/
const int JOINTS_HAND_Electric_Gripper = 1; /**< number of joints per hand*/
const int N_PHALANGE_Electric_Gripper = 0; /**< number of phalanges per finger*/

const double TOL_GRIP_QbSoftHand = 0; /**< tolerance on the grip in [mm] >*/

const  int HAND_FINGERS_QbSoftHand = 5; /**< number of fingers per hand>*/
const int JOINTS_HAND_QbSoftHand = 2; /**< number of joints per hand >*/
const int N_PHALANGE_QbSoftHand = 7;   /**< number of phalanges per finger*/
...
/** this struct defines the eletric gripper*/
typedef struct{
double maxAperture; /**< [mm] max aperture of the hand in [mm]*/
double minAperture; /**< [mm] min aperture of the hand in [mm]*/
double A1; /**< lenght of the finger in [mm]*/
double D3; /**< depth of the fingertip in [mm]*/
} electric_gripper;
...
/** This struct defines a QbSoftHand finger */
typedef struct{
std::string finger_name; /**< name of the finger >*/
DHparams finger_specs;   /**< the Denavit-Hatenberg parameters of the finger >*/
} qbsofthand_finger;

/** This struct defines a QbSoftHand >*/
typedef struct{
std::vector<HandPoses> FullHandPoses;
std::vector<HandPoses> PrecHandPoses;
bool prec;
vector<qbsofthand_finger> fingers; /**< fingers of the QbSoftHand >*/
double maxAp;                      /**< max aperture of the QbSoftHand in [mm] >*/
} qbsofthand;
