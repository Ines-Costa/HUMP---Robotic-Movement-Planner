...
#else
    int hand_code_right = scene->getHumanoid()->getHandRight();
    int hand_code_left = scene->getHumanoid()->getHandLeft();
    size_t joints_right_hand, joints_left_hand;

    if(hand_code_right == 2) joints_right_hand = JOINTS_HAND_Electric_Gripper;
    else if(hand_code_right == 3) joints_right_hand = JOINTS_HAND_QbSoftHand;

    if(hand_code_left == 2) joints_left_hand = JOINTS_HAND_Electric_Gripper;
    else if(hand_code_left == 3) joints_left_hand = JOINTS_HAND_QbSoftHand;

    this->rightFinalPosture = std::vector<double>(JOINTS_ARM+joints_right_hand);
    this->rightFinalPosture_diseng = std::vector<double>(JOINTS_ARM+joints_right_hand);
    this->rightFinalPosture_eng = std::vector<double>(JOINTS_ARM+joints_right_hand);
    this->leftFinalPosture = std::vector<double>(JOINTS_ARM+joints_left_hand);
    this->leftFinalPosture_diseng = std::vector<double>(JOINTS_ARM+joints_left_hand);
    this->leftFinalPosture_eng = std::vector<double>(JOINTS_ARM+joints_left_hand);
    this->rightFinalHand = std::vector<double>(joints_right_hand);
    this->leftFinalHand = std::vector<double>(joints_left_hand);
    this->initPosture_right = std::vector<double>(JOINTS_ARM+joints_right_hand);
    this->initPosture_left = std::vector<double>(JOINTS_ARM+joints_left_hand);
#endif
}
...
Problem::Problem(int planner_id, Movement* mov,Scenario* scene)
{
    ...
    if (hump){
        ...
        #else
            if(hand_code_right != hand_code_left){
                HUMotion::HUMPlanner::HANDS = true;
                if(hand_code_right == 2){
                    HUMotion::HUMPlanner::hand_fingers_right = HAND_FINGERS_Electric_Gripper;
                    HUMotion::HUMPlanner::joints_hand_right = JOINTS_HAND_Electric_Gripper;
                    HUMotion::HUMPlanner::n_phalange_right = N_PHALANGE_Electric_Gripper;
                }else if(hand_code_right == 3){
                    HUMotion::HUMPlanner::hand_fingers_right = HAND_FINGERS_QbSoftHand;
                    HUMotion::HUMPlanner::joints_hand_right = JOINTS_HAND_QbSoftHand;
                    HUMotion::HUMPlanner::n_phalange_right = N_PHALANGE_QbSoftHand;
                }

                if(hand_code_left == 2){
                    HUMotion::HUMPlanner::hand_fingers_left = HAND_FINGERS_Electric_Gripper;
                    HUMotion::HUMPlanner::joints_hand_left = JOINTS_HAND_Electric_Gripper;
                    HUMotion::HUMPlanner::n_phalange_left = N_PHALANGE_Electric_Gripper;
                }else if(hand_code_left == 3){
                    HUMotion::HUMPlanner::hand_fingers_left = HAND_FINGERS_Electric_Gripper;
                    HUMotion::HUMPlanner::joints_hand_left = JOINTS_HAND_Electric_Gripper;
                    HUMotion::HUMPlanner::n_phalange_left = N_PHALANGE_Electric_Gripper;
                }
            }
            else if(hand_code_right == hand_code_left){
                HUMotion::HUMPlanner::HANDS = false;
                if(hand_code_right == 2){
                    HUMotion::HUMPlanner::hand_fingers = HAND_FINGERS_Electric_Gripper;
                    HUMotion::HUMPlanner::joints_hand = JOINTS_HAND_Electric_Gripper;
                    HUMotion::HUMPlanner::n_phalange = N_PHALANGE_Electric_Gripper;
                }else if(hand_code_right == 3){
                    HUMotion::HUMPlanner::hand_fingers = HAND_FINGERS_QbSoftHand;
                    HUMotion::HUMPlanner::joints_hand = JOINTS_HAND_QbSoftHand;
                    HUMotion::HUMPlanner::n_phalange = N_PHALANGE_QbSoftHand;
                }
            }
        #endif
        #if MOVEIT==1
...
#else
    if(hand_code_right == 2 || hand_code_left == 2){
        electric_gripper e_hand = this->scene->getHumanoid()->getElectricGripper();
        std::vector<int> rk; this->scene->getHumanoid()->getRK(rk);
    ...
    if(hand_code_right == 3){
        qbsofthand qb_hand_right = this->scene-> getHumanoid()->getRightQbSoftHand();
        HUMotion::QbSoftHand hump_qbhand_right;
        bool prec = this->mov->getGrip();

        hump_qbhand_right.maxAperture = qb_hand_right.maxAp;
        qb_hand_right.prec = prec;
        hump_qbhand_right.prec = qb_hand_right.prec;
        vector<HUMotion::QbFinger> hump_fings_right = hump_qbhand_right.fingers;
        vector<qbsofthand_finger> fings_right = qb_hand_right.fingers;
        for(size_t i=0; i<fings_right.size();++i){
            qbsofthand_finger fing_right = fings_right.at(i);
            HUMotion::QbFinger hump_fing_right;
            hump_fing_right.name = fing_right.finger_name;
            hump_fing_right.finger_specs.a = fing_right.finger_specs.a;
            hump_fing_right.finger_specs.alpha = fing_right.finger_specs.alpha;
            hump_fing_right.finger_specs.d = fing_right.finger_specs.d;
            hump_fing_right.finger_specs.theta = fing_right.finger_specs.theta;
            hump_fings_right.push_back(hump_fing_right);
        }
        hump_qbhand_right.fingers = hump_fings_right;

        if(!prec){
          vector<HUMotion::HandPoses> hump_fings_right_full_poses = hump_qbhand_right.FullHandPoses;
          vector<HandPoses> fings_right_full_poses = qb_hand_right.FullHandPoses;
          for(size_t i=0; i<fings_right_full_poses.size();++i){
              HandPoses fing_right_full_pose = fings_right_full_poses.at(i);
              HUMotion::HandPoses hump_fing_right_full_pose;
              hump_fing_right_full_pose.dFF = fing_right_full_pose.dFF;
              hump_fing_right_full_pose.synergy = fing_right_full_pose.synergy;
              hump_fing_right_full_pose.hand_angles = fing_right_full_pose.hand_angles;
              hump_fings_right_full_poses.push_back(hump_fing_right_full_pose);
          }
          hump_qbhand_right.FullHandPoses = hump_fings_right_full_poses;
        }else{
          vector<HUMotion::HandPoses> hump_fings_right_prec_poses = hump_qbhand_right.PrecHandPoses;
          vector<HandPoses> fings_right_prec_poses = qb_hand_right.PrecHandPoses;
          for(size_t i=0; i<fings_right_prec_poses.size();++i){
              HandPoses fing_right_prec_pose = fings_right_prec_poses.at(i);
              HUMotion::HandPoses hump_fing_right_prec_pose;
              hump_fing_right_prec_pose.dFF = fing_right_prec_pose.dFF;
              hump_fing_right_prec_pose.synergy = fing_right_prec_pose.synergy;
              hump_fing_right_prec_pose.hand_angles = fing_right_prec_pose.hand_angles;
              hump_fings_right_prec_poses.push_back(hump_fing_right_prec_pose);
          }
          hump_qbhand_right.PrecHandPoses = hump_fings_right_prec_poses;
        }
        h_planner->setRightQbSoftHand(hump_qbhand_right);
    }

    if(hand_code_left == 3){
        qbsofthand qb_hand_left = this->scene-> getHumanoid()->getLeftQbSoftHand();
        HUMotion::QbSoftHand hump_qbhand_left;
        bool prec = this->mov->getGrip();

        hump_qbhand_left.maxAperture = qb_hand_left.maxAp;
        qb_hand_left.prec = prec;
        hump_qbhand_left.prec = qb_hand_left.prec;
        vector<HUMotion::QbFinger> hump_fings_left = hump_qbhand_left.fingers;
        vector<qbsofthand_finger> fings_left = qb_hand_left.fingers;
        for(size_t i=0; i<fings_left.size();++i){
            qbsofthand_finger fing_left = fings_left.at(i);
            HUMotion::QbFinger hump_fing_left;
            hump_fing_left.name = fing_left.finger_name;
            hump_fing_left.finger_specs.a = fing_left.finger_specs.a;
            hump_fing_left.finger_specs.alpha = fing_left.finger_specs.alpha;
            hump_fing_left.finger_specs.d = fing_left.finger_specs.d;
            hump_fing_left.finger_specs.theta = fing_left.finger_specs.theta;
            hump_fings_left.push_back(hump_fing_left);
        }
        hump_qbhand_left.fingers = hump_fings_left;

        if(!prec){
          vector<HUMotion::HandPoses> hump_fings_left_full_poses = hump_qbhand_left.FullHandPoses;
          vector<HandPoses> fings_left_full_poses = qb_hand_left.FullHandPoses;
          for(size_t i=0; i<fings_left_full_poses.size();++i){
              HandPoses fing_left_full_pose = fings_left_full_poses.at(i);
              HUMotion::HandPoses hump_fing_left_full_pose;
              hump_fing_left_full_pose.dFF = fing_left_full_pose.dFF;
              hump_fing_left_full_pose.synergy = fing_left_full_pose.synergy;
              hump_fing_left_full_pose.hand_angles = fing_left_full_pose.hand_angles;
              hump_fings_left_full_poses.push_back(hump_fing_left_full_pose);
          }
          hump_qbhand_left.FullHandPoses = hump_fings_left_full_poses;
        }else{
          vector<HUMotion::HandPoses> hump_fings_left_prec_poses = hump_qbhand_left.PrecHandPoses;
          vector<HandPoses> fings_left_prec_poses = qb_hand_left.PrecHandPoses;
          for(size_t i=0; i<fings_left_prec_poses.size();++i){
              HandPoses fing_left_prec_pose = fings_left_prec_poses.at(i);
              HUMotion::HandPoses hump_fing_left_prec_pose;
              hump_fing_left_prec_pose.dFF = fing_left_prec_pose.dFF;
              hump_fing_left_prec_pose.synergy = fing_left_prec_pose.synergy;
              hump_fing_left_prec_pose.hand_angles = fing_left_prec_pose.hand_angles;
              hump_fings_left_prec_poses.push_back(hump_fing_left_prec_pose);
          }
          hump_qbhand_left.PrecHandPoses = hump_fings_left_prec_poses;
        }
        h_planner->setLeftQbSoftHand(hump_qbhand_left);
    }
#endif
    }
...
int Problem::getHandCodeRight(){
    return this->scene->getHumanoid()->getHandRight();
}

int Problem::getHandCodeLeft(){
    return this->scene->getHumanoid()->getHandLeft();
}
...
void Problem::setGraspOffset(double value)
{

    this->grasp_offset=value;
}

void Problem::setRightGraspOffset(double value)
{

    this->right_grasp_offset=value;
}

void Problem::setLeftGraspOffset(double value)
{

    this->left_grasp_offset=value;
}

double Problem::getGraspOffset()
{

    return this->grasp_offset;
}
...
bool Problem::finalPostureFingers(int hand_id, int hand_code, int arm_code)
{
    ...
    if(prec){
        if(obj->getGripParams()){
            ...
            #else
                if(hand_code == 2)
                    d_obj = double(1.015) * obj->get_dFF() * 2.0 + TOL_GRIP_Electric_Gripper;
                else if(hand_code == 3)
                    d_obj = obj->get_dFF()+TOL_GRIP_QbSoftHand;
            #endif
        }else{
            #if HAND == 0 || HAND == 1
            ...
            #else
                if(hand_code == 2)
                    d_obj = double(1.015) * obj->getRadius() * 2.0 + TOL_GRIP_Electric_Gripper;
                else if(hand_code == 3)
                    d_obj = obj->getRadius()*2.0+TOL_GRIP_QbSoftHand;
            #endif
        }
        #if HAND==0
            ...
            if(obj->getGripParams()){
                ...
                #elif HAND == 3
                    if(hand_id != 2)
                        d_obj = min(hh->getRightQbSoftHand().maxAp, obj->getRadius()*2+TOL_GRIP);
                    else
                        d_obj = min(hh->getLeftQbSoftHand().maxAp, obj->getRadius()*2+TOL_GRIP);
                #endif
            }
        #else
            if(hand_code == 2){
                if(obj->getGripParams()){
                    d_obj = double(1.025) * obj->get_dFF() * 2.0 + TOL_GRIP_Electric_Gripper;
                }else{
                    d_obj = double(1.025) * obj->getRadius() * 2.0 + TOL_GRIP_Electric_Gripper;
                }
            }else if(hand_code == 3){
                if(obj->getGripParams()){
                    d_obj = obj->get_dFF()+TOL_GRIP_QbSoftHand;
                    ...
                    if(hand_id != 2)
                        d_obj = min(hh->getRightQbSoftHand().maxAp, obj->getRadius()*2+TOL_GRIP_QbSoftHand);
                    else
                        d_obj = min(hh->getLeftQbSoftHand().maxAp, obj->getRadius()*2+TOL_GRIP_QbSoftHand);
                }
            }
        #endif
    }
    ...
    if(hand_code == 2){
        ...
        else if(d_obj < hh->getElectricGripper().minAperture + TOL_GRIP_Electric_Gripper)
        {
            success = false;
            throw string("impossible to grasp the object ") + obj->getName() +
                    string(" with the grip ") + this->mov->getGripStr() +
        ...
        if(hand_id == 1){
            if(d_obj > hh->getRightQbSoftHand().maxAp)
            {
                success = false;
                throw string("impossible to grasp the object ") + obj->getName() +
                        string(" with the grip ") + this->mov->getGripStr() +
            ...
            if(d_obj > hh->getLeftQbSoftHand().maxAp)
            {
                success = false;
                throw string("impossible to grasp the object ") + obj->getName() +
                        string(" with the grip ") + this->mov->getGripStr() +
    ...
    try{
        ...
        inv_succ = this->invKinHand(d_obj,sols,hand_id, hand_code, arm_code);
        if (inv_succ){
            #if HAND == 0 || HAND == 1
                theta = sols.at(0);
                thetaT = sols.at(1);
    ...
    case 1: // right arm
            ...
            if(hand_code == 2){
                ...
                tol_grip = TOL_GRIP_Electric_Gripper;
            }else if(hand_code == 3){
                this->rightFinalHand.at(0) = synergy;
                this->rightFinalHand.at(1) = manipulation;
                this->rightFinalPosture.at(JOINTS_ARM) = synergy;
                this->rightFinalPosture.at(JOINTS_ARM +1) = manipulation;
                tol_grip = TOL_GRIP_QbSoftHand;
            }
        #endif
    ...
    case 2: // left arm
            ...
            this->leftFinalPosture_diseng.at(JOI    NTS_ARM+2)=theta;
            ...
            if(hand_code == 2){
                ...
                this->leftFinalHand.at(0) = synergy;
                this->leftFinalHand.at(1) = manipulation;
                this->leftFinalPosture.at(JOINTS_ARM) = synergy;
                this->leftFinalPosture.at(JOINTS_ARM + 1) = manipulation;
                tol_grip = TOL_GRIP_QbSoftHand;
            }
        #endif
...
bool Problem::invKinHand(double d_obj,std::vector<double>& sols, int hand_id, int hand_code, int arm_code)
{
    ...
    qbsofthand hand;
    if(hand_code == 2){
        A1 = hh->getElectricGripper().A1;
        minAp = hh->getElectricGripper().minAperture;
        maxAp = hh->getElectricGripper().maxAperture;
        d = minAp + d_obj;
        this->dFH = A1;
    }else if(hand_code == 3){
        if(hand_id == 1){
            hand = hh->getRightQbSoftHand();
            maxAp = hh->getRightQbSoftHand().maxAp;
        }
        else if(hand_id == 2){
            hand = hh->getLeftQbSoftHand();
            maxAp = hh->getLeftQbSoftHand().maxAp;
        }
        d = d_obj;
    }
#endif
    ...
    if(hand_code == 2){
        if (cnt < 100){success = true;}
        if(arm_code != 0)
            sols.at(0)=d - this->grasp_offset;
        else{
            if(hand_id ==1)
                sols.at(0) = d - this->right_grasp_offset;
            else if(hand_id == 2)
                sols.at(0) = d - this->left_grasp_offset;
        }
    }else if(hand_code == 3){
        std::vector<double> hand_angles;
        double new_d;
        if(arm_code != 0)
            new_d = d - this->grasp_offset;
        else{
            if(hand_id == 1)
                new_d = d - this->right_grasp_offset;
            else if(hand_id == 2)
                new_d = d - this->left_grasp_offset;
        }
        std::vector<double> main_hand_angles = handPose(&new_d, this->mov->getGrip(), hand_angles, hand);

        this->dFF = new_d;
        double Index_1 = (hand.fingers.at(1).finger_specs.a.at(4)/2) + (hand.fingers.at(1).finger_specs.a.at(5)) + (hand.fingers.at(1).finger_specs.a.at(6)/2);
        double Index_2 = (hand.fingers.at(1).finger_specs.a.at(6)/2) + (hand.fingers.at(1).finger_specs.a.at(7)) + (hand.fingers.at(1).finger_specs.a.at(8)/2);
        double Index_3 = (hand.fingers.at(1).finger_specs.a.at(8)/2) + (hand.fingers.at(1).finger_specs.a.at(9));
        double Tip = (hand.fingers.at(1).finger_specs.d.at(9));
        this->dFH = 50;// Index_1 * sin(hand_angles.at(4)*2) + Index_2 * sin(hand_angles.at(5)*2) + Index_3 * sin(hand_angles.at(6)*2) + Tip * cos(hand_angles.at(6));

        if(cnt < 100){success = true;}
        sols.at(0) = main_hand_angles.at(0);
        sols.at(1) = main_hand_angles.at(1);
    }
#endif
...
#if HAND != 0 || HAND != 1
   std::vector<double> Problem::handPose(double *d, bool prec, std::vector<double>& hand_angles, qbsofthand hand){
     std::vector<double> hand_main_angles = {0, 0};

    if(prec){
      hand.prec = true;
      double new_dFF = findClosest(*d, hand);
      bool angles_found = false;
      for (int i = 0; (i < hand.PrecHandPoses.size()) && (!angles_found) ; i++) {
        if(std::fabs(hand.PrecHandPoses.at(i).dFF - new_dFF) < 1e-10){
          angles_found = true;
          hand_angles = hand.PrecHandPoses.at(i).hand_angles;
          hand_main_angles.at(0) = hand.PrecHandPoses.at(i).synergy;
          hand_main_angles.at(1) = 1.0;
          *d = hand.PrecHandPoses.at(i).dFF;
        }
      }
    }else{
      hand.prec = false;
      double new_dFF = findClosest(*d, hand);
      bool angles_found = false;
      for (int i = 0; (i < hand.FullHandPoses.size()) && (!angles_found) ; i++) {
        if(std::fabs(hand.FullHandPoses.at(i).dFF - new_dFF) < 1e-10){
          angles_found = true;
          hand_angles = hand.FullHandPoses.at(i).hand_angles;
          hand_main_angles.at(0) = hand.FullHandPoses.at(i).synergy;
          hand_main_angles.at(1) = 0.0;
          *d = hand.FullHandPoses.at(i).dFF;
        }
      }
    }

    return hand_main_angles;
  }

  double Problem::findClosest(double value, qbsofthand hand){
    std::vector<HandPoses> array = (hand.prec? hand.PrecHandPoses : hand.FullHandPoses);

    int left = 0, right = array.size() - 1;
    while(left < right){
      if(abs(array.at(left).dFF - value) <= abs(array.at(right).dFF - value)){
         right --;
      }
      else{left++;}
    }
    return array.at(left).dFF;
  }
#endif
...
HUMotion::planning_result_ptr Problem::solve(HUMotion::hump_params &params)
{
    ...
    params.mov_specs.support_obj = "Shelf1";
    ...
    #else
        int hand_code;
        if((this->scene->getHumanoid()->getHandRight() == 2 && arm_code == 1) ||
           (this->scene->getHumanoid()->getHandLeft() == 2 && arm_code == 2)){
            // Electric Gripper
            hand_code = 2;
        }else if((this->scene->getHumanoid()->getHandRight() == 3 && arm_code == 1) ||
                 (this->scene->getHumanoid()->getHandLeft() == 3 && arm_code == 2)){
            // QbSoftHand
            hand_code = 3;
        }
    #endif
    ...
    if(mov_type!=1 && mov_type!=5){
        try{ // compute the final posture of the fingers according to the object involved in the movement
            this->finalPostureFingers(arm_code, hand_code, arm_code);
      ...
      case 2:
        obj = this->mov->getObjectLeft();
        pose = this->mov->getPoseLeft();
        // engaging info
        obj_eng = this->mov->getObjectEngLeft();
        eng = obj->getEngagePoint();
        eng1 = obj_eng->getEngagePoint();
        break;
      default:
    ...
    }else if((mov_type==1) && !moveSettings){
      tar_pose = {pose->getPos().Xpos, pose->getPos().Ypos, pose->getPos().Zpos, pose->getOr().roll, pose->getOr().pitch, pose->getOr().yaw};
    }
    ...
    case 0:// reach-to-grasp
        ...
        std::cout << "Tempo: " << this->exec_time << std::endl;
        break;
    case 1:// reaching
        if(!moveSettings){
...
HUMotion::planning_dual_result_ptr Problem::solve(HUMotion::hump_dual_params &params)
{
    this->solved = false;
    int arm_code =  this->mov->getArm();
    int mov_type_right = this->mov->getType(); 
    int mov_type_left = this->mov->getTypeLeft();
    int sceneID = this->scene->getID();
    //params.mov_specs_right.support_obj = "Shelf4";
    //params.mov_specs_left.support_obj = "Shelf4";
    #if HAND==0
        // Human Hand
        int hand_code = 0;
    #elif HAND == 1
        ...
        if(this->scene->getHumanoid()->getHandRight() == 2) hand_code_right = 2;
        else if(this->scene->getHumanoid()->getHandRight() == 3) hand_code_right = 3;

        if(this->scene->getHumanoid()->getHandLeft() == 2) hand_code_left = 2;
        else if(this->scene->getHumanoid()->getHandLeft() == 3) hand_code_left = 3;
    ...
    if(mov_type_right!=1 && mov_type_right!=5){
        try{ // compute the final posture of the fingers according to the object involved in the movement
            this->finalPostureFingers(1, hand_code_right, arm_code); // right hand
        }catch(const string message){throw message;}
    ...
    if(mov_type_left!=1 && mov_type_left!=5){
        try{ // compute the final posture of the fingers according to the object involved in the movement
            this->finalPostureFingers(2, hand_code_left, arm_code); // left hand
        }catch(const string message){throw message;}
    ...
    params.mov_specs_right.hand_code = hand_code_right;
    ...
    params.mov_specs_left.hand_code = hand_code_left;
    ...
    if(mov_type_right==1 && mov_type_left==1){
      //dual-arm reaching right and reaching left
      params.mov_specs_right.target = tar_pose_right;
      params.mov_specs_left.target = tar_pose_left;
      curr_time = this->GetTimeMs64();
      res = this->h_planner->plan_dual_move_move(params, initPosture_right,initPosture_left);
      this->exec_time = double(this->GetTimeMs64()-curr_time);
      std::cout << "Tempo: " << this->exec_time << std::endl;
