...
Movement::Movement(int type, int arm, objectPtr obj, posePtr pose)
{
...
Movement::Movement(int type_r, int type_l, int arm, posePtr pose_r, posePtr pose_l)
{
    this-> arm = arm;
    this->executed = false;
...
string Movement::getInfoLine()
{
    ...
    if(arm!=0)
    {
        ...
        if(arm == 1)
            ...
            return strType +", Arm: "+arm_info+", Object: "+obj_left->getName()+
                    ", Object Engaged: "+obj_eng_left->getName() + ", Pose: "+pose_left->getName()+
                    ", Grip Type: "+grip_str_left;
    }else{
        // dual arm
        string str_type = " ";
