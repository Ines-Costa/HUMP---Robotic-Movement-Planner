...
void Scenario::setTarget(int pos, targetPtr tg)
{

    this->targets_list.at(pos) = targetPtr(new Target(*tg.get()));
...
bool Scenario::getTargets(vector<targetPtr> &tgs)
{

    if(!this->targets_list.empty()){
        tgs = std::vector<targetPtr>(this->targets_list.size());
        std::copy(this->targets_list.begin(),this->targets_list.end(),tgs.begin());
        return true;
    }else{
        return false;
    }
...
void Scenario::addTarget(targetPtr target_ptr)
{
    this->targets_list.push_back(targetPtr(new Target(*target_ptr.get())));
}
...
targetPtr Scenario::getTarget(int pos)
{
    std::vector<targetPtr>::iterator ii = this->targets_list.begin();
    std::advance(ii,pos);
...
targetPtr Scenario::getTarget(string pose_name)
{

    targetPtr target = NULL;

    for(std::size_t i=0; i<this->targets_list.size();++i){
        string name = this->targets_list.at(i)->getName();
        if(boost::iequals(name,pose_name)){
            target=this->targets_list.at(i);
            break;
        }
    }
    return target;
