...
const double FINAL_TOL = 1e-4; /**< desired convergence tolerance */
...
const double AP = 25.0*static_cast<double>(M_PI)/180;//15.0*static_cast<double>(M_PI)/180; /**< aperture of the fingers when approaching to pick [rad] */
const double AP_PRISMATIC = -10; /**< aperture of the fingers when approaching to pick [mm] */
const double AP_SOFTHAND = 5.61309563*static_cast<double>(M_PI)/180;
...
typedef struct{
    ...
    bool prec;
    double maxAperture;
    vector<QbFinger> fingers; /**< fingers of the hand */
} QbSoftHand;
/* **** ***** **** **** */
...
typedef struct{
    ...
    int head_code; /**< the code of the head: 0 = no head, 1 = head*/
...
typedef struct{
    ...
    bool choose_retreat_final_value;
} boundaryConditions;
